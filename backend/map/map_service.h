...
class MapService {
 public:
  ...
  void CollectMapElementIds(const jmc_auto::common::PointENU &point,
                            double raidus, MapElementIds *ids) const;

  bool GetPathsFromRouting(const jmc_auto::routing::RoutingResponse &routing,
                           std::vector<jmc_auto::hdmap::Path> *paths) const;
  ...
  bool GetStartPoint(jmc_auto::common::PointENU *start_point) const;
 ...
 private:
  ...
  bool GetNearestLane(const double x, const double y,
                      jmc_auto::hdmap::LaneInfoConstPtr *nearest_lane,
