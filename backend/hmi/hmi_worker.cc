...
DEFINE_string(map_data_path, "/jmcauto/modules/map/data", "Path to map data.");

DEFINE_string(vehicle_data_path, "/jmcauto/modules/calibration/data",
...
using jmc_auto::common::util::FindOrNull;
using jmc_auto::common::util::StringTokenizer;
...
std::string TitleCase(const std::string &origin,
  ...
  return jmc_auto::common::util::PrintIter(parts);
}
...
Map<std::string, std::string> ListDirAsDict(const std::string &dir) {
  ...
  const auto subdirs = jmc_auto::common::util::ListSubPaths(dir);
  for (const auto &subdir : subdirs) {
    const auto subdir_title = TitleCase(subdir);
    const auto subdir_path = jmc_auto::common::util::StrCat(dir, "/", subdir);
...
void SetGlobalFlag(const std::string &flag_name, const ValueType &value,
  ...
  static constexpr char kGlobalFlagfile[] =
      "/jmcauto/modules/common/data/global_flagfile.txt";
  if (*flag != value) {
    *flag = value;
...
HMIWorker::HMIWorker() {
  ...
  config_.set_docker_image(jmc_auto::data::InfoCollector::GetDockerImage());
  ...
  *config_.mutable_available_vehicles() =
  ...
  jmc_auto::common::KVDB::Put("jmc_auto:dreamview:mode", status_.current_mode());
  ...
  if (!FLAGS_map_dir.empty()) {
    for (const auto &entry : config_.available_maps()) {
      ...
      if (entry.second == FLAGS_map_dir) {
        ...
        jmc_auto::common::KVDB::Put("jmc_auto:dreamview:map", entry.first);
        break;
      }
    }
  }
...
void HMIWorker::SubmitDriveEvent(const uint64_t event_time_ms,
  ...
  jmc_auto::common::DriveEvent drive_event;
...
void HMIWorker::ChangeToMap(const std::string &map_name) {
  ...
  jmc_auto::common::KVDB::Put("jmc_auto:dreamview:map", map_name);
...
void HMIWorker::ChangeToVehicle(const std::string &vehicle_name) {
  ...
  jmc_auto::common::KVDB::Put("jmc_auto:dreamview:vehicle", vehicle_name);
...
void HMIWorker::ChangeToMode(const std::string &mode_name) {
  ...
  jmc_auto::common::KVDB::Put("jmc_auto:dreamview:mode", mode_name);
