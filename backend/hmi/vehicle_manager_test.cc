...
TEST_F(VehicleManagerTest, Success) {
  ASSERT_TRUE(jmc_auto::common::util::EnsureDirectory(kTargetDir));

  EXPECT_TRUE(VehicleManager::instance()->UseVehicle(kTestVehicle));
  EXPECT_TRUE(jmc_auto::common::util::PathExists(
      StrCat(kTargetDir, "/vehicle_data.pb.txt")));

  ASSERT_TRUE(jmc_auto::common::util::RemoveAllFiles(kTargetDir));
}
