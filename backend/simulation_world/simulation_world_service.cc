...
using jmc_auto::common::DriveEvent;
...
using jmc_auto::common::time::ToSecond;
using jmc_auto::common::time::millis;
...
using jmc_auto::hdmap::Curve;
using jmc_auto::hdmap::Map;
using jmc_auto::hdmap::Path;
using jmc_auto::localization::Gps;
...
using jmc_auto::planning::StopReasonCode;
using jmc_auto::planning_internal::PlanningData;
...
using jmc_auto::relative_map::NavigationInfo;
...
std::string GetLogFileName() {
  ...
  char name_buffer[80];
  std::time(&raw_time);
  strftime(name_buffer, 80, "/data/dreamview/dv_autocar_%F_%H%M%S.csv",
           localtime(&raw_time));
...
Object::DisengageTypegear DeduceDisengageTypegear(const Chassis &chassis) {
  if (chassis.error_code() != Chassis::NO_ERROR) {
    return Object::GEAR_CHASSIS_ERROR;
  }
  switch (chassis.gear_location()) {
    case Chassis::GEAR_NEUTRAL:
      return Object::GEAR_NEUTRAL;
    case Chassis::GEAR_DRIVE:
      return Object::GEAR_DRIVE;
    case Chassis::GEAR_REVERSE:
      return Object::GEAR_REVERSE;
    case Chassis::GEAR_PARKING:
      return Object::GEAR_PARKING;
    default:
      return Object::GEAR_NONE;
  }
}
...
void UpdateTurnSignal(const jmc_auto::common::VehicleSignal &signal,
                      Object *auto_driving_car) {
  if (signal.turn_signal() == jmc_auto::common::VehicleSignal::TURN_LEFT) {
...
void SimulationWorldService::Update() {
  ...
  world_.set_timestamp(jmc_auto::common::time::AsInt64<millis>(Clock::Now()));
}
...
Json SimulationWorldService::GetUpdateAsJson(double radius) const {
  ...
  update["timestamp"] = jmc_auto::common::time::AsInt64<millis>(Clock::Now());
...
void SimulationWorldService::UpdateSimulationWorld(const Gps &gps) {
  ...
  double heading = jmc_auto::common::math::QuaternionToHeading(
...
void SimulationWorldService::UpdateSimulationWorld(const Chassis &chassis) {
  ...
  auto_driving_car->set_disengage_typegear(DeduceDisengageTypegear(chassis));
...
Object &SimulationWorldService::CreateWorldObjectIfAbsent(
  ...
  if (!jmc_auto::common::util::ContainsKey(obj_map_, id)) {
...
void SimulationWorldService::SetObstaclePolygon(
  ...
  using jmc_auto::common::util::PairHash;
...
void SimulationWorldService::UpdateSimulationWorld(
  ...
  if (traffic_light_detection.traffic_light_size() > 0) {
    ...
    signal->set_current_signal(
        jmc_auto::perception::TrafficLight_Color_Name(traffic_light.color()));
  } else {
    signal->set_current_signal("");
  }
}
...
void SimulationWorldService::UpdateMainStopDecision(
    const jmc_auto::planning::MainDecision &main_decision,
    double update_timestamp_sec, Object *world_main_decision) {
  jmc_auto::common::math::Vec2d stop_pt;
  ...
  if (main_decision.has_not_ready()) {
    ...
    // Normal stop.
    const jmc_auto::planning::MainStop &stop = main_decision.stop();
...
bool SimulationWorldService::LocateMarker(
    const jmc_auto::planning::ObjectDecisionType &decision,
    Decision *world_decision) {
  jmc_auto::common::PointENU fence_point;
  double heading;
...
void SimulationWorldService::FindNudgeRegion(
  ...
  std::vector<jmc_auto::common::math::Vec2d> points;
  ...
  const jmc_auto::common::math::Polygon2d obj_polygon(points);
  const jmc_auto::common::math::Polygon2d &nudge_polygon =
      obj_polygon.ExpandByDistance(std::fabs(decision.nudge().distance_l()));
  const std::vector<jmc_auto::common::math::Vec2d> &nudge_points =
        ...
        if (decision.has_stop() || decision.has_follow() ||
          ...
          if (decision.has_stop()) {
            ...
            for (auto obstacle_id : decision.stop().wait_for_obstacle()) {
              ...
              jmc_auto::common::util::split(obstacle_id, '_', &id_segments);
