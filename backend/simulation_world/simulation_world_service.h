...
class SimulationWorldService {
 public:
  ...
  void PublishMonitorMessage(
      jmc_auto::common::monitor::MonitorMessageItem::LogLevel log_level,
  ...
  const jmc_auto::hdmap::Map &GetRelativeMap() const;
 ...
 private:
  ...
  Object &CreateWorldObjectIfAbsent(
      const jmc_auto::perception::PerceptionObstacle &obstacle);
  void SetObstacleInfo(const jmc_auto::perception::PerceptionObstacle &obstacle,
                       Object *world_object);
  void SetObstaclePolygon(
      const jmc_auto::perception::PerceptionObstacle &obstacle,
      Object *world_object);
  void UpdatePlanningTrajectory(
      const jmc_auto::planning::ADCTrajectory &trajectory);
  bool LocateMarker(const jmc_auto::planning::ObjectDecisionType &decision,
                    Decision *world_decision);
  void FindNudgeRegion(const jmc_auto::planning::ObjectDecisionType &decision,
                       const Object &world_obj, Decision *world_decision);
  void UpdateDecision(const jmc_auto::planning::DecisionResult &decision_res,
                      double header_time);
  ...
  void UpdateMainChangeLaneDecision(const MainDecision &decision,
    ...
    if (decision.has_change_lane_type() &&
        (decision.change_lane_type() == jmc_auto::routing::ChangeLaneType::LEFT ||
         decision.change_lane_type() ==
             jmc_auto::routing::ChangeLaneType::RIGHT)) {
  ...
  void CreatePredictionTrajectory(
      const jmc_auto::prediction::PredictionObstacle &obstacle,
      Object *world_object);

  void DownsamplePath(const jmc_auto::common::Path &paths,
                      jmc_auto::common::Path *downsampled_path);

  void UpdatePlanningData(const jmc_auto::planning_internal::PlanningData &data);
  ...
  jmc_auto::hdmap::Map relative_map_;
